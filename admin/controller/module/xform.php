...
class ControllerModuleXform extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
             ...
             $formName = isset($this->request->post['formDesc'][$language_id]['formName'])?$this->request->post['formDesc'][$language_id]['formName']:'';
             ...
             if(isset($this->request->post['labels']) && is_array($this->request->post['labels'])) {
                 ...
                 foreach($this->request->post['labels'] as $languageId=>$lang_data) {
                   ...
                   $this->model_xform_xform->setFormLang($formId, $languageId, $lang_data, $lang_options, $lang_guidelines);
                 }
             }
...
//        $data['stores']=  array_merge(array(array('store_id'=>0,'name'=>$this->language->get('store_default'))),$data['stores']);
		...
		$row_total = $this->model_xform_xform->getTotalRecords($formId);
			...
			if($formInfo){
				 ...
				 if(isset($xform_langs) && is_array($xform_langs)) {
                    foreach($xform_langs as $languageId=>$lang_data) {
                       if($lang_data) {
                         $this->model_xform_xform->setFormLang($formId, $languageId, $lang_data['data'], $lang_data['options'], $lang_data['guidelines']);
                       }
                    }
                }
