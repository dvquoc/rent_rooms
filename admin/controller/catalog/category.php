...
class ControllerCatalogCategory extends Controller {
	...
	public function repair() {
		...
		if ($this->validateRepair()) {
			...
			$this->response->redirect($this->url->link('catalog/category', 'token=' . $this->session->data['token'].'&category_type='.$this->objects->object_type, 'SSL'));
		}
	...
	protected function getForm() {
		...
		$data['help_category_type'] = $this->language->get('help_category_type');
		...
		} elseif (!empty($category_info['category_type'])) {
			$data['category_type'] = $category_info['category_type'];
		}else {
			$data['category_type'] = $this->objects->object_type;
		}
	...
	protected function validateDelete() {
		...
		global $define_type_website;

		foreach ($this->request->post['selected'] as $category_id) {
			if(isset($define_type_website[$category_id]))
				$this->error['warning'] = $category_id." Đã được khai báo là loại website". $define_type_website[$category_id]." Nên bạn không thể xóa được";
		}
	...
	public function autocomplete() {
        ...
        $type_category = $this->objects->object_type;
		if (isset($this->request->get['filter_type']))
            $type_category = $this->request->get['filter_type'];
			$this->load->model('catalog/category');
			$filter_data = array(
				'filter_name' => $this->request->get['filter_name'],
				'filter_category_type' => $type_category,
				'sort'        => 'name',
				'order'       => 'ASC',
				'start'       => 0,
				'limit'       => 5
