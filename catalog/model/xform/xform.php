...
class ModelXformXform extends Model
{
 ...
 public function renderForm($formId,$data=array(), $isAdmin=false, $fromModule=false){
    ...
    if($isAdmin && $theme=='custom')
        $theme = 'classic'; // custom theme is not available for admin
	
	$output .= '<div class="form-wrapper">';
	if(!$formInfo['hideTitle'])
	    $output .= '<h3 class="title-module title-fuature text-center"><span>'.$formInfo['formName'].'</span></h3>';
	if($formInfo['formDescription'] && false)
	    $output .= '<p class="form-desc">'.$formInfo['formDescription'].'</p>';
    ...
    $hidden_replacers = array();
	 ...
	 foreach ($fields as $field ) {
		   ...
		   if(!$field['hide_label'] && $theme!="custom" && false) {
		   ...
		   if($field['field_type']=='hidden'){
			    ...
			    $css_class.=' hidden';
		        $element_html   .= '<input id="field-'.$field['cid'].'" class="form-item '.$css_class.'" name="data['.$field['cid'].']" value="'.(isset($data[$field['cid']])?$data[$field['cid']]:'').'" type="hidden" />';
		   }
		   ...
		   if($field['field_type']=='text' || $field['field_type']=='email'){
		        ...
		        $element_html   .= '<input placeholder="'.$lables[$field['cid']].'" id="field-'.$field['cid'].'" class="form-item '.$css_class.'" name="data['.$field['cid'].']" value="'.(isset($data[$field['cid']])?$data[$field['cid']]:'').'" type="text" />';
		   }
		   
		   if($field['field_type']=='paragraph'){
			    $css_class.=isset($field['field_options']['size'])?' '.$field['field_options']['size']:'';
			    $data[$field['cid']] = isset($data[$field['cid']])?$data[$field['cid']]:((isset($field['field_options']['hidden']) && $field['field_options']['hidden'])?str_replace($hidden_keywords,$hidden_replacers, $field['field_options']['hidden']):'');
				$element_html   .= '<textarea placeholder="'.$lables[$field['cid']].'" id="field-'.$field['cid'].'" class="form-item '.$css_class.'" name="data['.$field['cid'].']">'.(isset($data[$field['cid']])?$data[$field['cid']]:'').'</textarea>';
		   }
		   ...
		   if($field['field_type']=='copy'){
		    ...
		    if($field['field_type']=='dropdown'){
			   ...
			   $element_html   .= '<select id="field-'.$field['cid'].'" class="form-item '.$css_class.'" name="data['.$field['cid'].']">';
		   ...
		   if($field['field_type']=='checkboxes'){
			    ...
			    foreach ($field['field_options']['options'] as $inc=>$option){
			      ...
			      $option['label'] = isset($lang_options[$field['cid'].'_'.$inc])?$lang_options[$field['cid'].'_'.$inc]:$option['label'];
			      $element_html   .= '<label for="field-'.$field['cid'].'-'.$counter.'"><input id="field-'.$field['cid'].'-'.$counter.'" class="form-item '.$css_class.'" name="data['.$field['cid'].'][]" '.$checked.' value="'.htmlentities($value).'" type="checkbox" />&nbsp;'.html_entity_decode($option['label']).'</label>';
				  $counter++; 
			    }
				
				/* Option for other*/
				$checked=(isset($data[$field['cid']]['other']) && $data[$field['cid']]['other'])?'checked':'';
				$element_html   .=isset($field['field_options']['include_other_option'])?'<label for="field-'.$field['cid'].'-'.$counter.'"><input id="field-'.$field['cid'].'-'.$counter.'" class="form-item '.$css_class.'" name="data['.$field['cid'].'][other]" '.$checked.' value="1" type="checkbox" />&nbsp;'.$this->language->get('text_xform_other').'</label><input placeholder="'.$this->language->get('text_xform_other_placeholder').'" class="option-other" name="data['.$field['cid'].'][other_value]" value="'.(isset($data[$field['cid']]['other_value'])?$data[$field['cid']]['other_value']:'').'" type="text" />':'';
			    $element_html    .= '</div>';
		   ...
		   if($field['field_type']=='radio'){
			    ...
			    foreach ($field['field_options']['options'] as $inc=>$option){
			      ...
			      $option['label'] = isset($lang_options[$field['cid'].'_'.$inc])?$lang_options[$field['cid'].'_'.$inc]:$option['label'];
			      $element_html   .= '<label for="field-'.$field['cid'].'-'.$counter.'"><input id="field-'.$field['cid'].'-'.$counter.'" class="form-item '.$css_class.'" name="data['.$field['cid'].']" '.$checked.' value="'.htmlentities($value).'" type="radio" />&nbsp;'.html_entity_decode($option['label']).'</label>';
				  $counter++; 
			    }
		   ...
		   if($field['field_type']=='submit'){
			    $css_class.=' btn';
			   $element_html   .= '<div id="field-'.$field['cid'].'" class="xform-submit '.$css_class.'" name="submit" type="submit" />'.(isset($lables[$field['cid']])?$lables[$field['cid']]:'Submit').'</div>';
		   ...
		   if($field['field_type']=='address' && false) {
