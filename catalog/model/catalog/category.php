...
class ModelCatalogCategory extends Model {
	...
	public function getCategories($parent_id = 0,$data=array()) {
		$sql="SELECT * FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) WHERE c.parent_id = '" . (int)$parent_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "'  AND c.status = '1'";
		if(!empty($data['filter_category_type'])){
			$sql.=" AND category_type='".$data['filter_category_type']."'";
		}
